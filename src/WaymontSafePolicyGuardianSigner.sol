...
/// @notice Smart contract signer (via ERC-1271) for Safes wrapping an EOA and supporting changing the EOA or bypassing
/// this signer (for all Safes) from a global manager address as well as bypassing this signer after a timelock on a
/// specific Safe.
...
contract WaymontSafePolicyGuardianSigner is EIP712DomainSeparator {
    ...
    /// WARNING: If this variable is set to the zero address, wallets will not require signatures from either policy
    /// guardian address--so do NOT set this variable to the zero address unless you are sure you want to allow all
    /// transactions to bypass the policy guardian's firewall.
    /// The (primary or secondary) policy guardian must sign all transactions for a `Wallet`, unless the policy guardian
    /// is deactivated on a `Wallet` or if the primary `policyGuardian` is set to the zero address here, in which case
    /// all transactinos will bypass the policy guardian's firewall.
    /// The (primary or secondary) policy guardian can act as off-chain transaction policy node(s) permitting
    /// realtime/AI-based fraud detection, symmetric/access-token-based authentication mechanisms, and/or instant
    /// onboarding to the Waymont chain.
    /// The (primary or secondary) policy guardian can be disabled/enabled via a user-specified timelock on the
    /// `Wallet`.
    address public policyGuardian;

    /// @notice The secondary policy guardian address.
    /// WARNING: Even if the secondary guardian address is set, if the primary guardian address is not set, the `Wallet`
    /// contract does not require a signature from either policy guardian address.
    ...
    /// @notice For each `Safe`, custom timelock after which the policy guardian can be disabled (in seconds). If not
    /// set for a specific `Safe`, defaults to `DEFAULT_POLICY_GUARDIAN_TIMELOCK`.
    ...
    /// @param permanently Whether or not the policy guardian was globally disabled permanently (if not, if it can be
    /// re-enabled later on).
    ...
    /// Useful for sending security notifications to `Safe` owners if the policy guardian cannot find record of itself
    /// sending the transaction hash associated with a `DisablePolicyGuardianQueued` event.
    ...
    /// WARNING: Even if the secondary guardian address is set, if the primary guardian address is not set, the `Wallet`
    /// contract will not require signatures from either policy guardian address.
    ...
    /// @notice Globally disables the policy guardian: i.e., makes it so that any and ALL wallets will NOT require
    /// signatures from either policy guardian address.
    /// WARNING: Do NOT call this function unless you are sure you want to allow all transactions through the policy
    /// guardian's firewall on ALL `Safe`s using this `WaymontSafePolicyGuardianSigner`.
    function disablePolicyGuardianGlobally() external onlyPolicyGuardianManager {
        ...
        emit PolicyGuardianDisabledGlobally(false); // Emit with flag `permanently` equal to `false` since the policy
            // guardian can be re-enabled later on
    }

    /// @notice Permanently disables the policy guardian on all wallets.
    /// WARNING: Do NOT call this function unless you are sure you want to PERMANENTLY allow all transactions through
    /// the policy guardian's firewall on ALL `Safe`s using this `WaymontSafePolicyGuardianSigner`.
    function disablePolicyGuardianPermanently() external onlyPolicyGuardianManager {
        ...
        policyGuardian = address(0);
        emit PolicyGuardianChanged(address(0));
        emit PolicyGuardianDisabledGlobally(true); // Emit with flag `permanently` equal to `true` since the policy
            // guardian can NOT be re-enabled later on
    }
    ...
    /// Off chain policy guardian logic: if changing the timelock when the policy guardian is already enabled, require
    /// that the user waits for the old timelock to pass after queueing it off-chain.
    /// @param policyGuardianTimelock The desired timelock required to disable the policy guardian without the policy
    /// guardian's signature.
    function setPolicyGuardianTimelock(uint256 policyGuardianTimelock) external {
        require(
            policyGuardianTimelock >= MIN_POLICY_GUARDIAN_TIMELOCK,
            "Policy guardian timelock must be at least 15 minutes. Call disablePolicyGuardian to disable it."
        );
        require(
            policyGuardianTimelock <= MAX_POLICY_GUARDIAN_TIMELOCK,
            "Policy guardian timelock cannot be more than 180 days."
        );
    ...
    /// WARNING: Do NOT call this function unless you are sure you want to allow all transactions from
    /// `Safe(msg.sender)` through the policy guardian's firewall.
    /// Off chain policy guardian logic: require that the user waits for the old timelock to pass after queueing it
    /// off-chain.
    ...
    /// @notice Disable the policy guardian (by setting `policyGuardianDisabled[safe] = true`) after the on-chain
    /// timelock has passed (without needing a signature from the policy guardian).
    ...
    function disablePolicyGuardianWithoutPolicyGuardian(Safe safe, bytes calldata signatures) external {
        ...
        require(
            block.timestamp <= minExecutionTimestamp + QUEUED_SIGNATURE_EXPIRATION,
            "Queued signatures are only usable for 1 week until they expire."
        );
    ...
    function isValidSignature(bytes calldata _data, bytes memory _signature) external view returns (bytes4) {
        ...
        if (
            signer == _policyGuardian || (signer == secondaryPolicyGuardian && signer != address(0))
                || _policyGuardian == address(0) || policyGuardianPermanentlyDisabled
                || policyGuardianDisabled[Safe(payable(msg.sender))]

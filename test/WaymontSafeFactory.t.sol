...
import { WaymontSafePolicyGuardianSigner } from "../src/WaymontSafePolicyGuardianSigner.sol";
import { WaymontSafeAdvancedSigner } from "../src/WaymontSafeAdvancedSigner.sol";
import { WaymontSafeTimelockedRecoveryModule } from "../src/WaymontSafeTimelockedRecoveryModule.sol";
...
contract WaymontSafeFactoryTest is Test {
        ...
        internal
    {
        ...
        Enum.Operation operation = Enum.Operation.DelegateCall;
        {
            ...
            data = abi.encodeWithSelector(
                waymontSafeFactory.createAdvancedSigner.selector,
                safeInstance,
                underlyingOwners,
                underlyingThreshold,
                deploymentNonce
    ...
    function setUpWaymontSafe() public {
        ...
        // Safe params--initiating signers are the signers that will remove themselves in favor of the `AdvancedSigner`
        ...
        address predictedAdvancedSignerInstanceAddress;
        {
            ...
            predictedAdvancedSignerInstanceAddress = Clones.predictDeterministicAddress(
                waymontSafeFactory.advancedSignerImplementation(), salt, address(waymontSafeFactory)
            );
        }
        ...
        advancedSignerInstance = waymontSafeFactory.createAdvancedSigner(
            safeInstance, underlyingOwners, underlyingThreshold, deploymentNonce
        );
        ...
        address predictedTimelockedRecoveryModuleInstanceAddress;
        {
            ...
            predictedTimelockedRecoveryModuleInstanceAddress = Clones.predictDeterministicAddress(
        ...
        // Use Safe.execTransaction to call MultiSend.multiSend to add WaymontSafeAdvancedSigner to the Safe as a signer
        // using Safe.swapOwner, remove the extra signer(s) from the Safe with Safe.removeOwner, enable the
        // WaymontSafeTimelockedRecoveryModule on the Safe, deploy the WaymontSafeAdvancedSigner, and deploy the
        ...
        for (uint256 i = 0; i < underlyingOwners.length; i++) {
            assert(advancedSignerInstance.isOwner(underlyingOwners[i]));
        }
        ...
        timelockedRecoveryModuleInstance =
            WaymontSafeTimelockedRecoveryModule(predictedTimelockedRecoveryModuleInstanceAddress);
        ...
        internal
    {
        ...
        bytes memory packedOverlyingSignatures;
        {
            ...
            if (options.testInvalidUserSignature) {
                userSignature1[50] = userSignature1[50] == bytes1(0x55) ? bytes1(0x66) : bytes1(0x55);
            ...
            bytes memory packedUserSignatures = BOB > ALICE
                ? abi.encodePacked(userSignature1, userSignature2)
                : abi.encodePacked(userSignature2, userSignature1);
            ...
            bytes memory advancedSignerOverlyingSignatureData = abi.encodePacked(uint256(65 * 2), packedUserSignatures);
            ...
            if (address(advancedSignerInstance) > address(policyGuardianSigner)) {
                packedOverlyingSignatures = abi.encodePacked(
                    ...
                    advancedSignerOverlyingSignaturePointer,
                    policyGuardianOverlyingSignatureData,
                    advancedSignerOverlyingSignatureData
                );
            } else {
                packedOverlyingSignatures = abi.encodePacked(
                    advancedSignerOverlyingSignaturePointer,
    ...
    function _testDisablePolicyGuardianWithoutPolicyGuardian(bool testUnqueueingInstead) internal {
        ...
        bytes memory advancedSignerOverlyingSignaturePointer = abi.encodePacked(
            bytes32(uint256(uint160(address(advancedSignerInstance)))), uint256((65 * 2) + 32 + 65), uint8(0)
        );
        bytes memory advancedSignerOverlyingSignatureData = abi.encodePacked(uint256(65 * 2), packedUserSignatures);
        ...
        // Pack user signatures
        packedUserSignatures = BOB > ALICE
            ? abi.encodePacked(userSignature1, userSignature2)
            : abi.encodePacked(userSignature2, userSignature1);
        ...
        advancedSignerOverlyingSignatureData = abi.encodePacked(uint256(65 * 2), packedUserSignatures);
    ...
    function _testSocialRecovery(TestSocialRecoveryOptions memory options) internal {
        ...
        bytes memory data = abi.encodeWithSelector(
            advancedSignerInstance.swapOwner.selector,
